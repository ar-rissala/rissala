...
const pageCopy: Record<Locale, {
  ...
  fr: {
    metaTitle: 'Actualités | Rissala',
    metaDesc: "Articles d'actualités sur la finance éthique, la Zakat et l'investissement éthique.",
    title: "Actualités & Éducation",
    subtitle: "Analyses des marchés, actualités, et guides complets pour exceller.",
  ...
  en: {
    metaTitle: 'News | Rissala',
    metaDesc: "News articles on ethical finance, Zakat, and ethical investment.",
    title: "News & Education",
    subtitle: "Market analysis, news, and comprehensive guides.",
  ...
  ar: {
    metaTitle: 'الأخبار | رسالة',
    metaDesc: "مقالات إخبارية عن التمويل الأخلاقي، الزكاة والاستثمار الأخلاقي.",
    title: "الأخبار والتعليم",
    subtitle: "تحليلات الأسواق، الأخبار، والأدلة الشاملة.",
    ...
    <div className="container mx-auto px-4 lg:px-8 py-10 sm:py-12 lg:py-16">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
        {articles.length === 0 ? (
          ...
          articles.map((post, i) => (
            <Link key={post.slug} href={`/${lang}/actualites/${post.slug}`}>
              <Card className="overflow-hidden border-border/50 bg-card/60 hover:bg-card hover:border-primary/50 transition-all duration-300 group cursor-pointer flex flex-col h-full">
                <div className="h-36 sm:h-48 overflow-hidden relative">
                  ...
                  <ImageWithFallback src={post.image ?? "/images/actualites/default-finance.jpg"} alt={post.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" fallbackSrc="/images/actualites/default-finance.jpg" />
                </div>
                <CardHeader className="flex-1">
                  <div className="flex items-center gap-4 text-xs text-muted-foreground mb-3 font-medium">
                    <span className="flex items-center text-primary bg-primary/10 px-2 py-1 rounded-md"><Tag className="w-3 h-3 mr-1" /> {post.tag || "Actualité"}</span>
