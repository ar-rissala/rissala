...
export async function generateMetadata({ params }: ArticlePageProps): Promise<Metadata> {
  ...
  if (!article || article.category !== 'finance-islamique') return { title: "Article non trouvé" };
...
export async function generateStaticParams() {
  ...
  return articles
    .filter(a => a.category === 'finance-islamique')
    .map((article) => ({
      slug: article.slug,
    }));
}

export default async function FinanceArticlePage({ params }: ArticlePageProps) {
  const { slug } = await params;
  const article = getArticleBySlug(slug);

  if (!article || article.category !== 'finance-islamique') {
    notFound();
  }
